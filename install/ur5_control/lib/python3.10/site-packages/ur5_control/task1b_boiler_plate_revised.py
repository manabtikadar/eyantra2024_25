...
def detect_aruco(image):
    ...
    for i, corner in enumerate(corners):
            ...
            if area >= aruco_area_threshold:
                ...
                # Draw Axis
                R, _ = cv2.Rodrigues(rvec) #rotational_matrix

                # Extract angles (in radians) from the rotation matrix
                # angle_x = np.arctan2(rotation_matrix[2][1], rotation_matrix[2][2])  # Pitch
                # angle_y = np.arctan2(-rotation_matrix[2][0], np.sqrt(rotation_matrix[2][1]**2 + rotation_matrix[2][2]**2))  # Roll
                # angle_z = np.arctan2(rotation_matrix[1][0], rotation_matrix[0][0])  # Yaw
                ...
                angles = (np.degrees(roll), np.degrees(pitch), np.degrees(yaw))
...
class aruco_tf(Node):
    ...
    def publish_tf(self, marker_id, position, orientation):
        ...
        from_frame_rel = "base_link"                                                                        # frame from which transfrom has been sent
        to_frame_rel = f"cam_{marker_id}"                                                                     # frame to which transfrom has been sent
    ...
    def process_image(self):
        ...
        # Calculate quaternion from corrected angle
            ...
            r = R.from_euler('xyz', [0, 0,yaw], degrees=True)
            orientation = r.as_quat()  ###

            # Retrieve distance from RGB camera
            distance_from_rgb = distance_from_rgb_list[i] # Convert mm to m
            ...
            self.publish_tf(marker_id, (float(z),float(x),float(y)), orientation)
